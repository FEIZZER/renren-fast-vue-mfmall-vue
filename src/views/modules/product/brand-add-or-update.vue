<template>
  ...
  >
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()"
      ...
      <el-input v-model="dataForm.image" placeholder="品牌图片地址" type="file"></el-input>
      </el-form-item>-->
      <el-form-item label="品牌图片地址" prop="image">
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(
              `/mfmallproduct/brand/info/${this.dataForm.id}`
            ),
            method: "get",
    ...
    dataFormSubmit() {
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/mfmallproduct/brand/${!this.dataForm.id ? "save" : "update"}`
            ),
            method: "post",
